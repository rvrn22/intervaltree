...
namespace IntervalTree
{
    ...
    public struct Interval<T> where T : IComparable<T>
    {
        ...
        /// <summary>
        ///     Determines if two intervals overlap (i.e. if this interval starts before the other ends and it finishes after the
        ///     other starts)
        /// </summary>
    ...
    public class IntervalTree<T, TypeValue> where T : IComparable<T>
    {
        ...
        public IntervalTree(IEnumerable<KeyValuePair<Interval<T>, TypeValue>> elems)
        {
            ...
            comparer = ComparerUtil.GetComparer();
            keyvalueComparer = new KeyValueComparer<T, TypeValue>(comparer);
        }
        ...
        ///     If there is more than one interval starting at the same time/value, the intervalnode.Interval stores the start time
        ///     and the maximum end time of all intervals starting at the same value.
        ///     All end values (except the maximum end time/value which is stored in the interval node itself) are stored in the
        ///     Range list in decreasing order.
        ///     Note: this is okay for problems where intervals starting at the same time /value is not a frequent occurrence,
        ///     however you can use other data structure for better performance depending on your problem needs
        /// </summary>
        ...
        public bool Add(Interval<T> interval, TypeValue value)
        {
            ...
            Root = IntervalNode.Add(Root, interval, value, ref wasAdded, ref wasSuccessful);
            if (Root != null)
            {
        ...
        ///     If multiple identical intervals (starting at the same time and also ending at the same time) are allowed, this
        ///     function will delete one of them( see procedure DeleteIntervalFromNodeWithRange for details)
        ///     In this case, it is easy enough to either specify the (interval, value) pair to be deleted or enforce uniqueness by
        ///     changing the Add procedure.
        /// </summary>
        /// <param name="arg">The arg.</param>
        ...
        ///     If multiple intervals starting at the same time/value are found to overlap the specified interval, they are
        ///     returned in decreasing order of their End values.
        /// </summary>
        ...
        public void GetIntervalsOverlappingWith(Interval<T> toFind, ref List<KeyValuePair<Interval<T>, TypeValue>> list)
        {
            if (Root != null)
            {
                Root.GetIntervalsOverlappingWith(toFind, ref list);
            }
        }
        ...
        ///     The multiple intervals start at the specified value, they are sorted based on their End value (i.e. returned in
        ///     ascending order of their End values)
        /// </summary>
        /// <param name="arg">The arg.</param>
        /// <returns></returns>
        ...
        ///     Those intervals starting at the same time/value are sorted further based on their End value (i.e. returned in
        ///     ascending order of their End values)
        /// </summary>
        ...
        public void Print()
        {
            Visit((node, level) =>
            {
                ...
                Console.Write("{0}.{1}", node.Interval.ToString(), node.Max);
        ...
        private bool TryGetIntervalImpl(IntervalNode subtree, Interval<T> data, out TypeValue value)
        {
            if (subtree != null)
            {
                var compareResult = data.Start.CompareTo(subtree.Interval.Start);
                ...
                {
                    var kthIndex = subtree.Range.BinarySearch(new KeyValuePair<T, TypeValue>(data.End, default(TypeValue)), keyvalueComparer);
                    if (kthIndex >= 0)
                    {
        ...
        private class IntervalNode
        {
            ...
            public static IntervalNode Add(IntervalNode elem, Interval<T> interval, TypeValue value, ref bool wasAdded, ref bool wasSuccessful)
            {
                ...
                {
                    var compareResult = interval.Start.CompareTo(elem.Interval.Start);
                    ...
                    if (compareResult < 0)
                    {
                        ...
                        if (wasAdded)
                        {
                            ...
                            else if (elem.Balance == -2)
                            {
                                ...
                                {
                                    var elemLeftRightBalance = elem.Left.Right.Balance;
                    ...
                    else if (compareResult > 0)
                    {
                        ...
                        if (wasAdded)
                        {
                            ...
                            else if (elem.Balance == 2)
                            {
                                ...
                                {
                                    var elemRightLeftBalance = elem.Right.Left.Balance;
            ...
            {
                var maxRange = node.Interval.End;
                ...
                {
                    var leftMax = node.Left.Max;
                    var rightMax = node.Right.Max;
            ...
            /// <summary>
            ///     Gets the range of intervals stored in this.Range (i.e. starting at the same value 'this.Interval.Start' as the
            ///     interval stored in the node itself)
            ...
            public IEnumerable<KeyValuePair<Interval<T>, TypeValue>> GetRange()
            {
                if (Range != null)
                {
                    ...
                    {
                        var kth = new Interval<T>(Interval.Start, value.Key);
            ...
            public IEnumerable<KeyValuePair<Interval<T>, TypeValue>> GetRangeReverse()
            {
                if (Range != null && Range.Count > 0)
                {
                    ...
                    {
                        var kth = new Interval<T>(Interval.Start, Range[k].Key);
                        yield return new KeyValuePair<Interval<T>, TypeValue>(kth, Range[k].Value);
                    }
                }
            }
            ...
            {
                var cmp = arg.Start.CompareTo(node.Interval.Start);
                IntervalNode newChild = null;
            ...
            /// <summary>
            ///     Returns all intervals beginning at the specified start value. The intervals are sorted based on their End value
            ///     (i.e. returned in ascending order of their End values)
            /// </summary>
            /// <param name="subtree">The subtree.</param>
            /// <param name="data">The data.</param>
            /// <returns></returns>
            public static List<KeyValuePair<Interval<T>, TypeValue>> GetIntervalsStartingAt(IntervalNode subtree, T start)
            {
                if (subtree != null)
                {
                    var compareResult = start.CompareTo(subtree.Interval.Start);
                    if (compareResult < 0)
                    {
            ...
            ///     If multiple intervals starting at the same time/value are found to overlap, they are returned in decreasing order
            ///     of their End values.
            /// </summary>
            /// <param name="toFind">To find.</param>
            /// <param name="list">The list.</param>
            public void GetIntervalsOverlappingWith(Interval<T> toFind, ref List<KeyValuePair<Interval<T>, TypeValue>> list)
            {
                ...
                else
                {
                    ...
                    if (Interval.OverlapsWith(toFind))
                    {
                        ...
                        list.Add(new KeyValuePair<Interval<T>, TypeValue>(Interval, Value));
            ...
            public IEnumerable<KeyValuePair<Interval<T>, TypeValue>> GetIntervalsOverlappingWith(Interval<T> toFind)
            {
                ...
                else
                {
                    ...
                    if (Interval.OverlapsWith(toFind))
                    {
                        yield return new KeyValuePair<Interval<T>, TypeValue>(Interval, Value);
            ...
            ///     If the interval tree is used with unique intervals, this method removes the interval specified as an argument.
            ///     If multiple identical intervals (starting at the same time and also ending at the same time) are allowed, this
            ///     function will delete one of them.
            ///     In this case, it is easy enough to either specify the (interval, value) pair to be deleted or enforce uniqueness by
            ///     changing the Add procedure.
            /// </summary>
            ...
            private bool DeleteIntervalFromNodeWithRange(Interval<T> interval)
            {
                if (Range != null && Range.Count > 0)
                {
                    ...
                    else if (rangeCount > 12)
                    {
                        ...
                        var k = Range.BinarySearch(new KeyValuePair<T, TypeValue>(interval.End, default(TypeValue)), keyvalueComparer);
                        if (k >= 0)
                        {
                    ...
                    if (intervalPosition == 0)
                    {
                        Interval = new Interval<T>(Interval.Start, Range[0].Key);
                        Value = Range[0].Value;
                        Range.RemoveAt(0);
                    }
        ...
        private class KeyValueComparer<TKey, TValue> : IComparer<KeyValuePair<TKey, TValue>>
        {
            ...
            ///     <see cref="IntervalTree&lt;T, TypeValue&gt;.KeyValueComparer&lt;TKey, TValue&gt;" /> class.
            /// </summary>
